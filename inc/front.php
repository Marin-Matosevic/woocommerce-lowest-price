...
class Front
{
    ...
    public function __construct()
    {
        ...
        if (WPLP_DISPLAY_TYPE == 'alt') {
            ...
            add_action('woocommerce_product_meta_end', [$this, 'display_lowest_price_in_meta']);
        } else {

            add_filter('woocommerce_get_price_html', [$this, 'get_price_html'], 1000, 2);
        }
    }
    ...
    public function get_lowest_price($object_id, $regular_price)
    {
        ...
        if ($_custom_lowest_price = get_post_meta($object_id, '_custom_lowest_price', true)) {
            return $_custom_lowest_price;
        }
    ...
    public function get_price_html($price_html, $product)
    {
        ...
        if ($product->get_type() == 'variable') {
            ...
            if (!empty($prices['price'])) {
                ...
                $actual_price = wc_get_price_to_display($product, ['price' => $min_price]);
            }
        } else {
        ...
        $lowest_price_in_30_days = wc_get_price_to_display($product, ['price' => $this->get_lowest_price($product->get_id(), $regular_price)]);

        if (WPLP_DISPLAY_TYPE == 'text') {

            $price_html = '<span class="lowest_price">' . esc_html__('Lowest price in last 30 days', 'lowest-price') . ': <span class="lowest_amount">' . wc_price($lowest_price_in_30_days) . $product->get_price_suffix() . '</span></span><br />';
            $price_html .= '<span class="actual_price">' . esc_html__('Actual price', 'lowest-price') . ': <span class="actual_amount">' . wc_price($actual_price) . $product->get_price_suffix() . '</span></span>';
        } else {
    ...
    public function display_lowest_price_in_meta()
    {
        ...
        if ($product->get_type() == 'variable') {
            ...
            if ($min_reg_price === $max_reg_price && $min_price === $max_price) {
                ...
                $price = '<span class="lowest_amount">' . strip_tags(wc_price(wc_get_price_to_display($product, ['price' => $lowest_price_in_30_days])) . $product->get_price_suffix()) . '</span>';
            } else {

                $prices_arr = [
                    esc_html__('N/A', 'lowest-price')
                ];
                ...
                foreach ($prices['price'] as $k => $value) {
                    ...
                    if ($prices['regular_price'][$k] > $value) {
                        ...
                        $prices_arr[$k] = strip_tags(wc_price(wc_get_price_to_display($product, ['price' => $lowest_price_in_30_days])) . $product->get_price_suffix());
                    } else {

                        $prices_arr[$k] = esc_html__('N/A', 'lowest-price');
                    }
                }
        ...
        echo '<span class="lowest_price">' . esc_html__('Lowest price in last 30 days', 'lowest-price') . ': ' . $price . '</span>';
    }
    ...
    public function wp_enqueue_style()
    {
        ...
        wp_enqueue_script(self::$asset_name, Lowest_Price::$plugin_url . '/asset/public/script.js', ['jquery'], $plugin_version, false);
    }
}
